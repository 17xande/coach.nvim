...
function M.is_nil(val, msg)
	if val ~= nil then
		error(string.format("%s: expected nil, got %s", msg or "assertion failed", vim.inspect(val)), 2)
	end
end

function M.is_true(val, msg)
	if val ~= true then
		error(string.format("%s: expected true, got %s", msg or "assertion failed", vim.inspect(val)), 2)
	end
end

function M.is_false(val, msg)
	if val ~= false then
		error(string.format("%s: expected false, got %s", msg or "assertion failed", vim.inspect(val)), 2)
	end
end
