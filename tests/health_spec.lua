...
describe("health.report", function()
	...
	-- The negative-rule trigger check used to live here. It asked the parser whether
	-- each trigger named an action anything emits, and its best case was the arrow
	-- keys: a trigger naming `<Right>` never fired for as long as track-action
	-- reported that key as raw bytes, and nothing said so.
	--
	-- There is no parser to ask now, and no way to ask the question inside a running
	-- Neovim -- see `coach.emit`. Every trigger this repo ships is driven for real in
	-- track-action's `tests/vocabulary_spec.lua` instead, and `tests/emit_spec.lua`
	-- still checks here that they parse.
